...
void handleSerial()
{
  if (pinManager.isPinAllocated(hardwareRX))
    return;
  ...
  while (Serial.available() > 0)
  {
    ...
    case AdaState::Header_A:
      ...
      {
        sendBytes(); // Send LED data as TPM2 Data Packet
      }
      else if (next == 'o')
      {
        continuousSendLED = false; // Disable Continuous Serial Streaming
      }
      else if (next == 'O')
      {
        continuousSendLED = true; // Enable Continuous Serial Streaming
      }
      else if (next == '{')
      { // JSON API
