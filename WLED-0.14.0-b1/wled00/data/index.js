...
var hol = [
  ...
  [
    ...
    4,
    1,
    "https://initiate.alphacoders.com/download/wallpaper/516792/images/jpg/510921363292536",
  ], // 4th of July
  [
    0,
    0,
    1,
    1,
    "https://initiate.alphacoders.com/download/wallpaper/1198800/images/jpg/2522807481585600",
  ], // new year
];
...
function rgbBri(a) {
  return (
    0.2126 * parseInt(a.r) + 0.7152 * parseInt(a.g) + 0.0722 * parseInt(a.b)
  );
}
...
function setCSL(cs) {
  ...
  if (hasRGB && !isRgbBlack(cs.dataset)) {
    ...
    if (hasWhite && w > 0) {
      cs.style.background = `linear-gradient(180deg, ${rgbStr(
        cs.dataset
      )} 30%, rgb(${w},${w},${w}))`;
    }
  } else {
...
function onLoad() {
  ...
  applyCfg();
  if (cfg.comp.hdays) {
    //load custom holiday list
    fetch((loc ? `http://${locip}` : ".") + "/holidays.json", {
      // may be loaded from external source
      method: "get",
    })
      .then((res) => {
  ...
  loadPalettes(() => {
    ...
    loadFXData(() => {
      ...
      loadFX(() => {
        setTimeout(() => {
          // ESP8266 can't handle quick requests
          loadPalettesData(() => {
  ...
  if (
    localStorage.getItem("pcm") == "true" ||
    (!/Mobi/.test(navigator.userAgent) && localStorage.getItem("pcm") == null)
  )
    togglePcMode(true);
...
function presetError(empty) {
  ...
  var cn = `<div class="pres c" ${
    empty
      ...
      : 'onclick="pmtLast=0;loadPresets();" style="cursor:pointer;padding:8px;margin-top: 16px;"'
  }>`;
  if (empty) cn += `你还没有预设!`;
  else cn += `加载预设时出了问题!`;
  ...
  if (hasBackup) {
    cn += `<br><br>`;
    if (empty)
      cn += `但是，可以使用之前安装的备份数据.<br>
			(保存预设项将隐藏此内容并覆盖备份)`;
    else cn += `这是最后一个已知的正常状态的备份:`;
...
function populateQL() {
  ...
  if (pQL.length > 0) {
    ...
    for (var key of pQL || []) {
      cn += `<button class="btn btn-xs psts" id="p${key[0]}qlb" title="${
        key[2] ? key[2] : ""
      }" onclick="setPreset(${key[0]});">${key[1]}</button>`;
    }
...
function populatePresets(fromls) {
  ...
  for (var key of arr || []) {
    ...
    cn += `<div class="pname lstIname" onclick="setPreset(${i})">${
      isPlaylist(i) ? "<i class='icons btn-icon'>&#xe139;</i>" : ""
    }${pName(i)}
	<i class="icons edit-icon flr" id="p${i}nedit" onclick="tglSegn(${
      i + 100
    })">&#xe2c6;</i></div>
	<i class="icons e-icon flr" id="sege${i + 100}" onclick="expand(${
      i + 100
    })">&#xe395;</i>
...
function parseInfo(i) {
  ...
  if (name === "Dinnerbone")
    d.documentElement.style.transform = "rotate(180deg)"; // Minecraft easter egg
...
${
  i.opt & 0x100
    ? inforow(
        "Debug",
        '<button class="btn btn-xs" onclick="requestJson({\'debug\':' +
          (i.opt & 0x0080 ? "false" : "true") +
          '});"><i class="icons ' +
          (i.opt & 0x0080 ? "on" : "off") +
          '">&#xe08f;</i></button>'
      )
...
${inforow("正常运行时间", getRuntimeStr(i.uptime))}
...
${inforow("主控", i.arch + " " + i.core + " (" + i.lwip + ")")}
</table>`;
  gId("kv").innerHTML = cn;
...
function populateSegments(s) {
  ...
  for (var inst of s.seg || []) {
    ...
    let sg = gId(`seg${i}`);
    let exp = sg
      ? sg.classList.contains("expanded") || (i === 0 && cfg.comp.segexp)
      : false;
    ...
    let segp = `<div id="segp${i}" class="sbs">
      ...
      inst.on ? "act" : ""
		...
		<div class="sliderwrap il">
			<input id="seg${i}bri" class="noslide" onchange="setSegBri(${i})" oninput="updateTrail(this)" max="255" min="1" type="range" value="${
      inst.bri
    }" />
    ...
    let stoY = inst.stopY;
    let rvXck = `<label class="check revchkl">反 ${
      isM ? "" : "方向"
    }<input type="checkbox" id="seg${i}rev" onchange="setRev(${i})" ${
      inst.rev ? "checked" : ""
    }><span class="checkmark"></span></label>`;
    let miXck = `<label class="check revchkl">Mirror<input type="checkbox" id="seg${i}mi" onchange="setMi(${i})" ${
      inst.mi ? "checked" : ""
    }><span class="checkmark"></span></label>`;
    let rvYck = "",
      miYck = "";
    if (isM) {
      rvYck = `<label class="check revchkl">Reverse<input type="checkbox" id="seg${i}rY" onchange="setRevY(${i})" ${
        inst.rY ? "checked" : ""
      }><span class="checkmark"></span></label>`;
      miYck = `<label class="check revchkl">Mirror<input type="checkbox" id="seg${i}mY" onchange="setMiY(${i})" ${
        inst.mY ? "checked" : ""
      }><span class="checkmark"></span></label>`;
    }
    ...
    cn += `<div class="seg lstI ${i == s.mainseg ? "selected" : ""} ${
      exp ? "expanded" : ""
    }" id="seg${i}">
	<label class="check schkl">
		<input type="checkbox" id="seg${i}sel" onchange="selSeg(${i})" ${
      inst.sel ? "checked" : ""
    }>
		<span class="checkmark"></span>
	</label>
	<i class="icons e-icon frz" id="seg${i}frz" onclick="event.preventDefault();tglFreeze(${i});">&#x${
	...
	<div class="segin" id="seg${i}in">
		<input type="text" class="ptxt noslide" id="seg${i}t" autocomplete="off" maxlength=32 value="${
      inst.n ? inst.n : ""
    }" placeholder="Enter name..."/>
		<table class="infot segt">
		<tr>
			<td>${isM ? "Start X" : "开始 LED"}</td>
			<td>${
        isM
          ? cfg.comp.seglen
            ? "Width"
		...
		<tr>
			<td><input class="noslide segn" id="seg${i}s" type="number" min="0" max="${
      (isM ? mw : ledCount) - 1
    }" value="${staX}" oninput="updateLen(${i})" onkeydown="segEnter(${i})"></td>
			<td><input class="noslide segn" id="seg${i}e" type="number" min="0" max="${
      (isM ? mw : ledCount) - (cfg.comp.seglen ? staX : 0)
    }" value="${
      stoX - (cfg.comp.seglen ? staX : 0)
    }" oninput="updateLen(${i})" onkeydown="segEnter(${i})"></td>
			<td style="text-align:revert;">${
        isM ? miXck + "<br>" + rvXck : ""
      }<input class="noslide segn ${
      isM ? "hide" : ""
    }" id="seg${i}of" type="number" value="${
      inst.of
      ...
      isM
        ? "<tr><td>Start Y</td><td>" +
          (cfg.comp.seglen ? "Height" : "Stop Y") +
          "</td><td></td></tr>" +
          "<tr>" +
          '<td><input class="noslide segn" id="seg' +
          i +
          'sY" type="number" min="0" max="' +
          (mh - 1) +
          '" value="' +
          staY +
          '" oninput="updateLen(' +
          ...
          '<td><input class="noslide segn" id="seg' +
          i +
          'eY" type="number" min="0" max="' +
          (mh - (cfg.comp.seglen ? staY : 0)) +
          '" value="' +
          (stoY - (cfg.comp.seglen ? staY : 0)) +
          '" oninput="updateLen(' +
          i +
          ')" onkeydown="segEnter(' +
          i +
		...
		<tr>
			<td><input class="noslide segn" id="seg${i}grp" type="number" min="1" max="255" value="${
      inst.grp
    }" oninput="updateLen(${i})" onkeydown="segEnter(${i})"></td>
			<td><input class="noslide segn" id="seg${i}spc" type="number" min="0" max="255" value="${
      inst.spc
    }" oninput="updateLen(${i})" onkeydown="segEnter(${i})"></td>
		...
		<label class="check revchkl" id="seg${i}lbtm">
			${isM ? "Transpose" : "镜面效果"}
			<input type="checkbox" id="seg${i}${isM ? "tp" : "mi"}" onchange="${
      (isM ? "setTp(" : "setMi(") + i
    })" ${isM ? (inst.tp ? "checked" : "") : inst.mi ? "checked" : ""}>
			<span class="checkmark"></span>
		</label>
		<div class="del">
  ...
  for (var i = 0; i <= lSeg; i++) {
    ...
    if (!gId(`seg${i}sel`).checked && gId("selall"))
      gId("selall").checked = false; // uncheck if at least one is unselected.
  }
  if (segCount < 2) gId(`segd${lSeg}`).style.display = "none";
  if (
    !isM &&
    !noNewSegs &&
    (cfg.comp.seglen ? parseInt(gId(`seg${lSeg}s`).value) : 0) +
      parseInt(gId(`seg${lSeg}e`).value) <
      ledCount
  )
    gId(`segr${lSeg}`).style.display = "inline";
...
function populateNodes(i, n) {
  ...
  if (n.nodes) {
    ...
    for (var o of n.nodes) {
      if (o.name) {
        var url = `<button class="btn" title="${
          o.ip
        }" onclick="location.assign('http://${o.ip}');">${bname(o)}</button>`;
        urows += inforow(
          url,
          `${btype(o.type)}<br><i>${o.vid == 0 ? "N/A" : o.vid}</i>`
        );
        nnodes++;
      }
    }
    ...
    if (stop - start > 1 && stopY - startY > 1) {
      ...
      if (sE) {
        ...
        if (seg) {
          ...
          // unhide mapping for 1D effects (| in name)
...
function updateUI() {
  ...
  gId("kwrap").style.display =
    hasRGB && !hasCCT /*&& ccfg.picker*/ ? "block" : "none"; // Kelvin slider
...
function updateSelectedPalette(s) {
  ...
  var selPaletteInput = parent.querySelector(
  ...
  if (selectedPalette)
    parent.querySelector(`.lstI[data-id="${s}"]`).classList.add("selected");
...
function updateSelectedFx() {
  ...
  var selEffectInput = parent.querySelector(
    `input[name="fx"][value="${selectedFx}"]`
  );
  ...
  if (selectedEffect) {
    ...
    for (const seg of segs)
      if (selectedName.indexOf("\u25A6") < 0) seg.classList.remove("hide");
      else seg.classList.add("hide");
    var segs = gId("segcont").querySelectorAll(`div[data-snd="si"]`);
    for (const seg of segs)
      if (
        selectedName.indexOf("\u266A") < 0 &&
        selectedName.indexOf("\u266B") < 0
      )
        seg.classList.add("hide");
      else seg.classList.remove("hide"); // also "♫ "?
  }
}

function displayRover(i, s) {
  gId("rover").style.transform =
    i.live && s.lor == 0 && i.liveseg < 0
      ? "translateY(0px)"
      : "translateY(100%)";
...
function cmpP(a, b) {
  ...
  return n.localeCompare((b[1].playlist ? "<" : y) + b[1].n, undefined, {
    numeric: true,
  });
}
...
function makeWS() {
  ...
  ws = new WebSocket(
    (window.location.protocol == "https:" ? "wss" : "ws") +
      "://" +
      (loc ? locip : window.location.hostname) +
      "/ws"
  );
...
function setEffectParameters(idx) {
  ...
  var slOnOff =
    effectPars.length == 0 || effectPars[0] == ""
      ? []
      : effectPars[0].split(",");
  var coOnOff =
    effectPars.length < 2 || effectPars[1] == ""
      ? []
      : effectPars[1].split(",");
  var paOnOff =
    effectPars.length < 3 || effectPars[2] == ""
      ? []
      : effectPars[2].split(",");
  ...
  for (let i = 0; i < nSliders; i++) {
    ...
    if (
      (!controlDefined && i < (idx < 128 ? 2 : nSliders)) ||
      (slOnOff.length > i && slOnOff[i] != "")
    ) {
  ...
  if (slOnOff.length > 5) {
    ...
    for (let i = 0; i < 3; i++) {
      if (5 + i < slOnOff.length && slOnOff[5 + i] !== "") {
        ...
        gId("optLabel" + i).innerHTML =
          slOnOff[5 + i] == "!" ? "Option" : slOnOff[5 + i].substr(0, 16);
  ...
  for (let i = 0; i < gId("csl").children.length; i++) {
    ...
    if (coOnOff.length > i && coOnOff[i] != "") {
      ...
      cslCnt++;
    } else if (!controlDefined) {
      // if no controls then all buttons should be shown for color 1..3
      btn.classList.remove("hide");
      btn.dataset.hide = 0;
      btn.innerHTML = `${i + 1}`;
      if (!cslCnt || oCsel == i) selectSlot(i); // select 1st displayed slot or old one
  ...
  if (
    !controlDefined ||
    (paOnOff.length > 0 && paOnOff[0] != "" && isNaN(paOnOff[0]))
  ) {
    ...
    if (paOnOff.length > 0 && paOnOff[0] != "!") pall.innerHTML = paOnOff[0];
      ...
      pall.innerHTML =
        '<i class="icons sel-icon" onclick="tglHex()">&#xe2b3;</i> 调色板';
  } else {
    // disable palette list
    pall.innerHTML =
      '<i class="icons sel-icon" onclick="tglHex()">&#xe2b3;</i> 没有使用调色板';
...
function requestJson(command = null) {
  ...
  if (command) {
    ...
    if (req.length > 500 && lastinfo && lastinfo.arch == "esp8266")
      useWs = false; // esp8266 can only handle 500 bytes
  }
...
function toggleNl() {
  ...
  if (nlA) {
    showToast(
      `定时器启动. 你的灯将在 ${nlDur} 分钟 ${nlMode ? "后" : "之后"} ${
        nlTar > 0 ? "打开" : "关闭"
      }.`
    );
  } else {
...
function toggleSync() {
  ...
  else showToast("此灯和网络中的其他灯将不再同步.");
...
function toggleLiveview() {
  ...
  if (isM) {
    ...
    gId("mliveview2D").style.transform = isLv
      ? "translateY(0px)"
      : "translateY(100%)";
  }
...
function toggleNodes() {
  ...
  gId("nodes").style.transform = isNodes
    ? "translateY(0px)"
    : "translateY(100%)";
...
function makeSeg() {
  ...
  if (lu > 0) {
    var pend =
      parseInt(gId(`seg${lu - 1}e`).value, 10) +
      (cfg.comp.seglen ? parseInt(gId(`seg${lu - 1}s`).value, 10) : 0);
	...
	<div class="segin">
		...
		<table class="segt">
          ...
          isM
			...
			<tr>
				<td><input class="noslide segn" id="seg${lu}s" type="number" min="0" max="${
    isM ? mw - 1 : ledCount - 1
  }" value="${ns}" oninput="updateLen(${lu})" onkeydown="segEnter(${lu})"></td>
...
function resetUtil(off = false) {
  gId("segutil").innerHTML =
    `<div class="seg btn btn-s ${
      off ? "off" : ""
    }" style="border-radius:24px;padding:0;">` +
    '<label class="check schkl"><input type="checkbox" id="selall" onchange="selSegAll(this)"><span class="checkmark"></span></label>' +
    `<div class="segname" ${
      off ? "" : 'onclick="makeSeg()"'
    }><i class="icons btn-icon">&#xe18a;</i>添加分段</div></div>`;
}
...
function refreshPlE(p) {
  ...
  if (!plEDiv) return;
  var content = '<div class="first c">播放列表编辑器</div>';
...
function makeP(i, pl) {
  ...
  if (pl) {
    ...
    content = `<div id="ple${i}" style="margin-top:10px;"></div><label class="check revchkl">打乱顺序
	<input type="checkbox" id="pl${i}rtgl" onchange="plR(${i})" ${
      plJson[i].r || rep < 0 ? "checked" : ""
    }>
	<span class="checkmark"></span>
</label>
<label class="check revchkl">无限重复
	<input type="checkbox" id="pl${i}rptgl" onchange="plR(${i})" ${
      rep > 0 ? "" : "checked"
    }>
	<span class="checkmark"></span>
</label>
<div id="pl${i}o1" style="display:${rep > 0 ? "block" : "none"}">
<div class="c">Repeat <input class="noslide" type="number" id="pl${i}rp" oninput="plR(${i})" max=127 min=0 value=${
      rep > 0 ? rep : 1
    }> times</div>
<div class="sel">End preset:<br>
<div class="sel-p"><select class="sel-ple" id="pl${i}selEnd" onchange="plR(${i})" data-val=${
      plJson[i].end ? plJson[i].end : 0
    }>
    ...
    if (Array.isArray(lastinfo.maps) && lastinfo.maps.length > 1) {
      ...
      for (const k of lastinfo.maps || [])
        content += `<option value="${k}"${
          i > 0 && pJson[i].ledmap == k ? " selected" : ""
        }>${k == 0 ? "Default" : "ledmap" + k + ".json"}</option>`;
      content += "</select></div></div>";
    }
  }

  return `<input type="text" class="ptxt noslide ${
    i == 0 ? "show" : ""
  }" id="p${i}txt" autocomplete="off" maxlength=32 value="${
    i > 0 ? pName(i) : ""
  }" placeholder="Enter name..."/>
...
<div class="c m6">保存到ID <input class="noslide" id="p${i}id" type="number" oninput="checkUsed(${i})" max=250 min=1 value=${
...
<div class="c">
	...
	${
    i > 0
      ? '<button class="btn btn-p" id="p' +
        i +
        'del" onclick="delP(' +
        i +
        ')"><i class="icons btn-icon">&#xe037;</i>Delete'
      : '<button class="btn btn-p" onclick="resetPUtil()">取消'
  }</button>
</div>
  ...
  return `<div class="plentry">
	...
	<tr>
		<td width="80%" colspan=2>
			<div class="sel-p"><select class="sel-pl" onchange="plePs(${p},${i},this)" data-val="${
    plJson[p].ps[i]
  }" data-index="${i}">
	...
	<tr>
		<td class="c" width="40%"><input class="noslide segn" type="number" placeholder="Duration" max=6553.0 min=0.2 step=0.1 oninput="pleDur(${p},${i},this)" value="${
    plJson[p].dur[i] / 10.0
  }">s</td>
		<td class="c" width="40%"><input class="noslide segn" type="number" placeholder="Transition" max=65.0 min=0.0 step=0.1 oninput="pleTr(${p},${i},this)" value="${
    plJson[p].transition[i] / 10.0
  }">s</td>
...
function makePlUtil() {
  if (pNum < 2) {
    showToast("你需要至少2个预设来制作播放列表!"); //return;
  }
  let p = gId("putil");
  p.classList.remove("staybot");
  p.classList.add("pres");
  p.innerHTML = `<div class="presin expanded" id="seg100">${makeP(
    0,
    true
  )}</div></div>`;
  refreshPlE(0);
...
function resetPUtil() {
  ...
  p.innerHTML =
    `<button class="btn btn-s" onclick="makePUtil()" style="float:left;"><i class="icons btn-icon">&#xe18a;</i>预设</button>` +
...
function setSeg(s) {
  ...
  if (gId(`seg${s}grp`)) {
    // advanced options, not present in new segment dialog (makeSeg())
    var grp = parseInt(gId(`seg${s}grp`).value);
    var spc = parseInt(gId(`seg${s}spc`).value);
    var ofs = parseInt(gId(`seg${s}of`).value);
    obj.seg.grp = grp;
...
function setPalette(paletteId = null) {
  if (paletteId === null) {
    paletteId = parseInt(
      d.querySelector('#pallist input[name="palette"]:checked').value
    );
  } else {
    d.querySelector(
      `#pallist input[name="palette"][value="${paletteId}"]`
    ).checked = true;
  }
...
function setPreset(i) {
  ...
  if (
    pJson &&
    pJson[i] &&
    (!pJson[i].win || pJson[i].win.indexOf("Please") <= 0)
  ) {
...
function saveP(i, pl) {
  ...
  if (!gId(`p${i}cstgl`).checked) {
    ...
    if (pl) {
      ...
      if (gId(`p${i}lmp`) && gId(`p${i}lmp`).value !== "")
        obj.ledmap = parseInt(gId(`p${i}lmp`).value);
    }
  }
...
function updatePSliders() {
  ...
  gId("sliderS").nextElementSibling.style.backgroundImage =
    "linear-gradient(90deg, #aaa -15%, rgb(" +
    c.r +
    "," +
    c.g +
    "," +
  ...
  gId("sliderV").nextElementSibling.style.backgroundImage =
    "linear-gradient(90deg, #000 -15%, rgb(" +
    c.r +
    "," +
    c.g +
    "," +
...
function hideModes(txt) {
  for (let e of gId("fxlist").querySelectorAll(".lstI") || []) {
    ...
    let f = false;
    if (txt === "2D")
      f = iT.indexOf("\u25A6") >= 0 && iT.indexOf("\u22EE") < 0; // 2D && !1D
...
function showNodes() {
  gId("buttonNodes").style.display =
    lastinfo.ndc > 0 && (wW > 797 || (wW > 539 && wW < 720)) ? "block" : "none";
}
...
function togglePcMode(fromB = false) {
  ...
  if (!fromB && ((wW < 1250 && lastw < 1250) || (wW >= 1250 && lastw >= 1250)))
    return;
  openTab(0, true);
  if (wW < 1250) {
    pcMode = false;
